...
fn run_web(port: u16, bind: String, log_level: &LogLevel) -> i32 {
    ...
    if use_gui {
        ...
        // url_rx and shutdown_tx are consumed by the native GUI event loop.
        // When the gui feature is disabled, drop them to suppress warnings.
        #[cfg(not(feature = "gui"))]
        drop((url_rx, shutdown_tx));
