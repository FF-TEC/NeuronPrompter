...
fn render_platform(window: &Window, rgba: &[u8], width: u32, height: u32) -> Result<(), String> {
    ...
    // The casts below are safe: splash images are at most 960x360 pixels,
    // well within i32/usize range on all supported platforms.
    let w = width as usize;
    let h = height as usize;
    let stride_usize = stride as usize;
    let mut argb_data = vec![0u8; stride_usize * h];
    for y in 0..h {
        for x in 0..w {
            let src = (y * w + x) * 4;
            let dst = y * stride_usize + x * 4;
            let (r, g, b, a) = (
                u32::from(rgba[src]),
                u32::from(rgba[src + 1]),
                u32::from(rgba[src + 2]),
    ...
    let img_width = f64::from(width);
    let img_height = f64::from(height);
    ...
    gtk_window.connect_draw(move |widget, cr| {
        ...
        let sx = f64::from(alloc.width()) / img_width;
        let sy = f64::from(alloc.height()) / img_height;
        cr.scale(sx, sy);
