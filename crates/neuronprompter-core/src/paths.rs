...
mod tests {
    ...
    fn sanitize_path_allows_current_dir_component() {
        ...
        let p = std::path::Path::new("/tmp/./safe");
        // This may or may not succeed depending on whether /tmp/safe exists,
        // but it should NOT fail with PathTraversal
        let result = sanitize_path(p);
        if let Err(e) = &result {
            // Should never be PathTraversal for "." paths
            assert!(
